...
export default function ContentCard({
  ...
  return (
    <Link
      href={href}
      className="group grid gap-4 grid-cols-1 sm:grid-cols-2 rounded-2xl border border-black/10 bg-surface p-4 shadow-sm transition-colors hover:bg-black/[.03] dark:border-white/10"
    >
      <div className="grid-cols-1 sm:grid-cols-2 relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-black/[.06] dark:bg-white/[.06]">
        <LazyImage
          src={imageSrc}
          alt=""
          fill
      ...
      <div className="min-w-0 grid-cols-1 sm:grid-cols-2 w-full">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="truncate text-2xl font-semibold tracking-tight">{title}</h3>
          {badge ? (
